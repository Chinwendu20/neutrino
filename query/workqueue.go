...
//
// NOTE: This is part of the heap.Interface implementation.
func (w *testWorkQueue) Len() int { return len(w.tasks) }

// Less returns whether the item in the priority queue with index i should sort
// before the item with index j.
//
// NOTE: This is part of the heap.Interface implementation.
func (w *testWorkQueue) Less(i, j int) bool {
	return w.tasks[i].Index() < w.tasks[j].Index()
}
...
//
// NOTE: This is part of the heap.Interface implementation.
func (w *testWorkQueue) Swap(i, j int) {
	w.tasks[i], w.tasks[j] = w.tasks[j], w.tasks[i]
}
...
// Peek returns the first item in the queue.
func (w *testWorkQueue) Peek() interface{} {
	return w.tasks[0]
}

func (w *testWorkQueue) Pop() interface{} {
	n := len(w.tasks)
	x := w.tasks[n-1]
	w.tasks[n-1] = nil
	w.tasks = w.tasks[0 : n-1]
