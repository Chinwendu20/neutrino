...
type blockManager struct { // nolint:maligned
	...
	hdrTipToResponse map[int32]*hdrQueryResponse
}
...
func newBlockManager(cfg *blockManagerCfg) (*blockManager, error) {
	...
	bm := blockManager{
		...
		hdrTipToResponse:    make(map[int32]*hdrQueryResponse),
	}
	...
	bm.writeBatchSignal = sync.NewCond(&bm.writeBatchMtx)
...
func (b *blockManager) getUncheckpointedCFHeaders(
	...
	// QueryBatch all peers for the responses.
	...
	for {
		...
		case err := <-errChan:
			...
			case err == query.ErrWorkManagerShuttingDown:
				return
			case err != nil:
				log.Errorf("QueryBatch finished with error before "+
...
func (b *blockManager) startSync(peers *list.List) {
	...
	for e := peers.Front(); e != nil; e = enext {
		...
		//TODO(Maureen): Perharps use peer timeout info for this
...
func (b *blockManager) handleHeadersMsg(hmsg *HeadersMsg) {
	...
	log.Debugf("Length of headers %v is %v", hmsg.Peer.Addr(), len(msg.Headers))
	for i, blockHeader := range msg.Headers {
		...
		if prevHash.IsEqual(&blockHeader.PrevBlock) {
			...
			log.Debugf("Total work from known chain: %v", knownWork)
	...
	//TODO(Maureen): only change header tip if final height is greater than the last height
