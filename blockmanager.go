...
func newBlockManager(cfg *blockManagerCfg) (*blockManager, error) {
	...
	// interval.
	genesisHeader, err := cfg.RegFilterHeaders.
		FetchHeaderByHeight(0)
	if err != nil {
		return nil, err
	}
	...
	header, height, err := cfg.blkHdrProcessor.store.ChainTip()
	if err != nil {
		return nil, err
	}

	bm.cfg.blkHdrProcessor.nextCheckpt = bm.cfg.blkHdrProcessor.
		findNextHeaderCheckpoint(int32(height))
	bm.cfg.blkHdrProcessor.headerList.ResetHeaderState(headerlist.Node{
		Header: *header,
		Height: int32(height),
	})
	bm.headerTip = height
	...
	fh, err := cfg.blkHdrProcessor.store.FetchHeaderByHeight(bm.filterHeaderTip)
	if err != nil {
		return nil, err
	}
...
func (b *blockManager) Start() {
	...
	log.Debug("Starting block manager")
	b.wg.Add(2)
	go b.blockHandler()
	go func() {
		defer b.wg.Done()
...
func (b *blockManager) handleNewPeerMsg(peers *list.List, sp *ServerPeer) {
	...
	// the new peer.
	_, height, err := b.cfg.blkHdrProcessor.store.ChainTip()
	if err != nil {
...
waitForHeaders:
	...
	log.Infof("Starting cfheaders sync for filter_type=%v", filterType)
	...
	for len(goodCheckpoints) == 0 && lastHeight >= wire.CFCheckptInterval {
		...
		if minCheckpointHeight(allCFCheckpoints) < lastHeight {
			...
			allCFCheckpoints = b.getCheckpts(&bestHash, filterType)
	...
	b.getCheckpointedCFHeaders(
		goodCheckpoints, store, filterType,
	)
...
func (b *blockManager) getCheckpointedCFHeaders(checkpoints []*chainhash.Hash,
	...
	for currentInterval < uint32(len(checkpoints)) {
		...
		stopHeader, err := b.cfg.blkHdrProcessor.store.
...
func (b *blockManager) rollBackToHeight(height uint32) error {
	...
	for uint32(bs.Height) > height {
		...
		if uint32(bs.Height) <= regHeight {
			newFilterTip, err := b.cfg.RegFilterHeaders.RollbackLastBlock(
				newTip)
			if err != nil {
				return err
			}
...
func (b *blockManager) detectBadPeers(headers map[string]*wire.MsgCFHeaders,
	...
	header, err := b.cfg.blkHdrProcessor.store.
...
func (b *blockManager) startSync(peers *list.List) {
	...
	if bestPeer != nil {
		...
		if b.cfg.blkHdrProcessor.nextCheckpt != nil && int32(
			bestHeight) < b.cfg.blkHdrProcessor.nextCheckpt.Height {
...
func (b *blockManager) handleHeadersMsg(hmsg *headersMsg) {
	...
	for i, blockHeader := range msg.Headers {
		...
		if valid {
			...
			// the next one.
			_, _, err := b.cfg.blkHdrProcessor.store.FetchHeader(
				&blockHash)
			if err == nil {
				continue
			}
			...
			for j, reorgHeader := range msg.Headers[i:] {
				...
				err = b.cfg.blkHdrProcessor.checkHeaderSanity(
					...
					b.cfg.blkHdrProcessor.headerList,
				)
				if err != nil {
			...
			for j := uint32(prevNode.Height); j > backHeight; j-- {
				if knownEl != nil {
					...
					knownHead, _, err = b.cfg.blkHdrProcessor.store.
						FetchHeader(&knownHead.PrevBlock)
					if err != nil {
	...
	if len(headerWriteBatch) > 0 {
		...
		// is atomic.
		err := b.cfg.blkHdrProcessor.store.WriteHeaders(
			headerWriteBatch...)
		if err != nil {
...
func (b *blockManager) NotificationsSinceHeight(
	...
	for i := height + 1; i <= bestHeight; i++ {
		header, err := b.cfg.blkHdrProcessor.
			store.FetchHeaderByHeight(i)
		if err != nil {
			return nil, 0, err
		}
